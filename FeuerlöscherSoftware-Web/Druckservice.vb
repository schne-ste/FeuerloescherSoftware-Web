...
Public Class Druckservice
    ...
    Private apiUrl As String = "http://10.122.122.4/loescher/api/index.php"
    Private apiToken As String = "123"
    Private druckername_global As String = Ini.ReadValue("Drucker", "BondruckerName", "", Application.StartupPath & "\config.ini")
    Private druckername_etikett As String = Ini.ReadValue("Drucker", "EtikettenDruckerName", "", Application.StartupPath & "\config.ini")
    ...
    Private Async Sub apiTimer_Tick(sender As Object, e As EventArgs) Handles apiTimer.Tick
        ...
        Log("Timer beendet - nächster Lauf in 5 Sekunden.")
        apiTimer.Start()
    End Sub
    ...
    Private Async Function CheckAbholschein() As Task
        ...
        For Each item In data
            ...
            Dim zeit = item("zeitstempel").ToString()

            Log($"Drucke Abholschein: {nummer} / Kunde: {name} / Typ: {typ} / Betrag: {preis}")
            Print_Abholschein(name, nummer, typ, preis, bezahlt, defekt, zeit, druckername_global)
    ...
    Private Async Function CheckRechnungen() As Task
        Dim url As String = apiUrl & "?route=/rechnungen&token=" & apiToken
        ...
        For Each item In data
            ...
            Print_Rechnung("Voller Preis", name, preis, anzahl, rnr, druckername_global, adresse, plzort)
